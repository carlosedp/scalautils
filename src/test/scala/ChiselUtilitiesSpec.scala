import chisel3._
import chiseltest._
import com.carlosedp.scalautils.Observer
...
class ObserverTest extends AnyFlatSpec with ChiselScalatestTester with Matchers {
  ...
  it should "honor Wiring Transform for BoringUtils with Wire" in {
    test(new TopWrapper) { c =>
      c.observed_wire.expect(42.U)
    }
  }
  it should "honor Wiring Transform for BoringUtils with Vector" in {
    test(new TopWrapper) { c =>
      c.clock.step()
