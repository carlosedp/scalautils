...
/**
 * Add this trait to your wrapper module to observe on submodule signals using the `observe` method.
 * This avoids cluttering the original module with signals that would be used only by the test harness.
 */
trait Observer { this: Module =>

  /**
   * The method `observe` allows one to bring to the wrapped module (top), signals from
   * instantiated sub-modules so they can be tested by peek/poke chiseltest.
   *
   * Usage:
   *
   * {{{
   ...
   * @return a signal with the same format to be tested on Top module's spec.
   */
